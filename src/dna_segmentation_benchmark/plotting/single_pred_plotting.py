...
def group_annotation(annotation):
    ...
    return [(annotation[start], start, end) for start, end in zip(group_start_indices, group_end_indices)]
...
def plot_pred_vs_gt_enhanced(
    ...
    if reading_frame is not None:
        ...
        grouped_reading_frame = [
            (val, start, end)
            for val, start, end in group_annotation(reading_frame)
    ...
    for region_type, start, end in grouped_prediction:
        ...
        if region_type == 8:  # Intergenic line
            ...
            ds["color"].append(
                MISMATCH_HIGHLIGHT_COLOR if segment_mismatches else color
            )  # Line color based on mismatch
            ds["alpha"].append(BASE_ALPHA)
    ...
    # Ground Truth
    gt_quads_renderer = p.quad(
        source=gt_quad_source,
        name="gt_quads",
        legend_label="GT Exon/Intron",
        left="left",
        right="right",
        top="top",
        bottom="bottom",
        ...
        alpha="alpha",
        line_color="black",
        line_width=LINE_WIDTH_QUAD_MATCH,
    )

    gt_lines_renderer = p.multi_line(
        source=gt_line_source,
        name="gt_lines",
        legend_label="GT Intergenic",
        xs="x",
        ys="y",
        color="color",
        alpha="alpha",
    ...
    pred_quads_renderer = p.quad(
        source=pred_quad_source,
        name="pred_quads",
        legend_label="Pred Exon/Intron",
        left="left",
        right="right",
        top="top",
        bottom="bottom",
        ...
        line_width="line_width",
    )  # line_color/width from source

    pred_lines_renderer = p.multi_line(
        source=pred_line_source,
        name="pred_lines",
        legend_label="Pred Intergenic",
        xs="x",
        ys="y",
        color="color",
        alpha="alpha",
    ...
    if grouped_reading_frame:  # Check if there's data to plot after filtering
        rf_quads_renderer = p.quad(
            source=rf_quad_source,
            name="rf_quads",
            legend_label="Reading Frame",
            left="left",
            right="right",
            top="top",
            bottom="bottom",
