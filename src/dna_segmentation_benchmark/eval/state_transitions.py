...
def _compute_state_change_errors(
    ...
    """Compute how state transitions in GT are mispredicted across all labels."""
    ...
    failed_transition_mask = (nuc_transitions[:, 0, 0] == nuc_transitions[:, 1, 0]) & \
                                     (nuc_transitions[:, 0, 1] != nuc_transitions[:, 1, 1])

    failed_transitions = nuc_transitions[failed_transition_mask]

    for failed_transition in failed_transitions:
    ...
    for label_id,transition_failure_tuples in per_label_failures.items():
        if len(transition_failure_tuples) == 0:
            ...
            transition_matrices[label_id] = np.zeros((num_labels, num_labels), dtype=np.int64)
            continue

        gt_transition_ids, pred_transition_ids = zip(*transition_failure_tuples)

        transition_failure_matrix = confusion_matrix(
            gt_transition_ids, 
            pred_transition_ids,
            labels=sorted(list(per_label_failures.keys()))
        )
        transition_matrices[label_id] = transition_failure_matrix.astype(np.int64)
